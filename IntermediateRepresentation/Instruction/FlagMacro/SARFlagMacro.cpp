...
// All rights reserved.
////////////////////////////////////////////////////////////////////////////////
#include "SARFlagMacro.h"


SARFlagMacro::SARFlagMacro(ExpressionPtr _operand, ExpressionPtr _count, ExpressionPtr _result) : FlagMacro(SARFLAGS)
{
...
SARFlagMacro::~SARFlagMacro(void)
{
	/*delete operand;
	delete count;
	delete result;*/
...
std::string SARFlagMacro::getInstructionString(){
	...
	return macroString.substr(0, macroString.length() - 2) + ") = SARFLAGS("
			+ operand->getExpressionString() + ", " + count->getExpressionString()
			+ ", " + result->getExpressionString() + ")";
}

//std::vector<Expression*>* SARFlagMacro::getUsedElements(){
...
void SARFlagMacro::getUsedElements(std::vector<ExpressionPtr>& usedElements_out){
...
InstructionPtr SARFlagMacro::deepcopy(){
	...
	return std::make_shared<SARFlagMacro>(operandCopy, countCopy, resultCopy);
}
